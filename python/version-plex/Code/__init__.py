    ...
    try:
        if not itemlist:
            if caller_item_serialized is None:
                ...
                caller_item = Item(channel= channel_name, action= action)
            else:
                caller_item = Item()
                caller_item.fromurl(caller_item_serialized)
                Log.Info("Entrando en canal para ejectuar "+caller_item.channel+"."+action)
                Log.Info("caller_item_serialized="+caller_item_serialized)
                
            Log.Info("caller_item=" + caller_item.tostring())
            
            from core import channeltools
            if '.' in caller_item.channel:
                pack,modulo = caller_item.channel.split('.') #ejemplo: platformcode.platformtools
                channelmodule = channeltools.get_channel_module(modulo,pack)
            else:
                channelmodule = channeltools.get_channel_module(caller_item.channel)

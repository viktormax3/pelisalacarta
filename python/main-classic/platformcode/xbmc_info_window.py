...
class InfoWindow(xbmcgui.WindowXMLDialog):
    ...
    def get_tmdb_tv_data(self, text, season=0, episode=0):
        ...
        if season and episode:
            if not "seasons" in self.result or self.result["seasons"] =="":
            ...
            if episode > self.otmdb.result.get("seasons")[season-1]["episode_count"]: episode = self.otmdb.result.get("seasons")[season]["episode_count"]
            ...
            if episode_info.get("temporada_poster"): self.result["thumbnail"] = episode_info.get("temporada_poster")
            if self.otmdb.result.get("overview"): self.result["overview"] =  self.otmdb.result.get("overview")
            ...
            if episode_info.get("episodio_imagen"): self.result["fanart"] = episode_info.get("episodio_imagen")
            if episode_info.get("episodio_sinopsis"): self.result["overview"] = episode_info.get("episodio_sinopsis")

...
class SettingsWindow(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        for c in self.list_controls:
            ...
            if not "color" in c: c["color"] = "0xFF0066CC"
            ...
            if ctype == "text": hidden = c["hidden"] if type(c["hidden"]) == bool else (True if c["hidden"].lower() == 'true' else False)
            ...
            if ctype == "bool":
                #Creamos el control
                control = xbmcgui.ControlRadioButton(self.ContolsX - 10, -100, self.ContolsWidth + 10, self.height_control, label=label, font=self.font, textColor=color,
            ...
            #Control "text"
                ...
                #Creamos el control
                control = xbmcgui.ControlEdit(self.ContolsX, -100, self.ContolsWidth - 5, self.height_control, label, font=self.font, isPassword=hidden, textColor=color,
                                              focusTexture=os.path.join(self.mediapath, 'ChannelSettings', 'MenuItemFO.png'),
                                              noFocusTexture=os.path.join(self.mediapath, 'ChannelSettings', 'MenuItemNF.png'))
                #Lo añadimos a la ventana                             
                self.addControl(control)
            ...
            #Control "list"
                ...
                control = xbmcgui.ControlButton(self.ContolsX, -100, self.ContolsWidth, self.height_control, label, font=self.font, textOffsetX=0, textColor=color,
                                                focusTexture=os.path.join(self.mediapath, 'ChannelSettings', 'MenuItemFO.png'),
                                                noFocusTexture=os.path.join(self.mediapath, 'ChannelSettings', 'MenuItemNF.png'))
                         
                label = xbmcgui.ControlLabel(self.ContolsX, -100, self.ContolsWidth - 30, self.height_control, lvalues[value], font=self.font, textColor=color, alignment=4 | 1)

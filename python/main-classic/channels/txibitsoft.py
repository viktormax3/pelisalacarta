    ...
    if not "series" in item.url :
        ...
        if len(matches)==0:
                ...
                if len(matches)==0:
                    ...
                    show= "http://imgur.com/21Oty9A.jpg"+"|"+item.thumbnail+"|"+sinopsis
        ...
        if len(matches)==0:
         ...
         try:
            subdata_imdb =scrapertools.find_single_match(data,'<li class="b_algo">(.*?)h="ID.*?Series')
         except:
            pass
         ...
         urlremotetbdb = "https://api.themoviedb.org/3/find/"+imdb_id+"?api_key=2e2160006592024ba87ccdf78c28f49f&external_source=imdb_id&language=es"

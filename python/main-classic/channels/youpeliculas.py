...
def mainlist(item):
    ...
    itemlist.append( item.clone (title = "[Anime] Novedades/Actualizadas", action = "lista",  url = host+'/you/load_data', extra={'post':'type=anime', 'page_current':0, 'max_pages':1}))
    itemlist.append( item.clone (title = "[Infantiles] Novedades/Actualizadas", action = "lista",  url = host+'/you/load_data', extra={'post':'type=series-animadas', 'page_current':0, 'max_pages':1}))
    itemlist.append( item.clone (title = "[Dramas] Novedades/Actualizadas", action = "lista",  url = host+'/you/load_data', extra={'post':'type=dramas', 'page_current':0, 'max_pages':1}))
...
def lista (item):
    ...
    for li in matches:
        ...
        for vidId, scrapedurl, scrapedtitle, scrapedthumbnail in match:
            itemlist.append( Item(channel=item.channel, action='findvideos' , title=scrapedtitle , url=host+'/you/load-options', thumbnail=scrapedthumbnail, contentTitle = scrapedtitle, extra='vid='+vidId+'&ep='+vidId+'&type=movie' ))
        ...
        for scrapedurl, scrapedtitle, scrapedthumbnail in match:
            itemlist.append( Item(channel=item.channel, action='lista' , title=scrapedtitle+' (LISTADO)' , url=host+'/you/load_data', thumbnail=scrapedthumbnail, contentTitle = scrapedtitle+' (LISTADO)', extra={'post':'type=seasons&category='+scrapedurl, 'page_current':0, 'max_pages':1} ))
        # series
    ...
    if itemlist !=[]:
        ...
        matches = re.compile(patron, re.DOTALL).findall(data)
        if matches:
            itemlist.append(item.clone(title="Pagina Siguiente", action='lista', url=item.url, extra={'post':item.extra['post']+'&total='+matches[0][0], 'page_current':1, 'max_pages': int(matches[0][1]) }))
        else:
            if item.extra['page_current']+1<item.extra['max_pages']:
                itemlist.append(item.clone(title="Pagina Siguiente", action='lista', url=item.url, extra={'post':item.extra['post'], 'page_current':item.extra['page_current']+1, 'max_pages': item.extra['max_pages'] }))
...
def episodios (item):
    ...
    if int(item.temporada)>1 and item.moreTemp!=False:
        hist = scrapertools.find_single_match(item.url,'.co/(.*?)-temp-'+item.temporada)
        itemlist.append( Item(channel=item.channel, action='lista' , title='Temporadas Anteriores' , url=host+'/you/load_data', contentTitle = 'Temporadas Anteriores' , extra={'post':'type=seasons&category='+hist, 'page_current':0, 'max_pages':1}  ))
	
    matches = re.compile(patron,re.DOTALL).findall(data)
    for capId, cap, nameCap in matches:
        itemlist.append( Item(channel=item.channel, action='findvideos' , title='[Temp. '+item.temporada+'] Cap. '+cap+'.- '+nameCap , url=host+'/you/load-options', thumbnail=item.thumbnail, contentTitle = '[Temp.]'+item.temporada+'Cap. '+cap+'.- '+nameCap, extra='vid='+item.extra+'&ep='+capId+'&type=episodes' ))
...
def findvideos(item):
    logger.info ()
    itemlist=[]
    data = httptools.downloadpage(item.url,item.extra).data
    #data = re.sub(r"'|\n|\r|\t|&nbsp;|<br>", "", data)
	
    listado = scrapertools.find_single_match(data,'var sources = {(.*?)}, sel_source =')
    jsonVideos = jsontools.load_json('{'+listado+'}');
    lenguaje = scrapertools.find_single_match(data,'var labeli = {(.*?)};')	
    jsonLang = jsontools.load_json('{'+lenguaje+'}');
    #calidad = scrapertools.find_single_match(data,'var label = {(.*?)}.*?var labeli')
    #jsonQual = jsontools.load_json('{'+calidad+'}');
	
    for lang, cali in jsonVideos.items():
        for qual, links in cali.items():
            for link in links:
                vidId=link.split('_', 1 )
                #logger.info ('peliculasalacarta.channel.youpeliculas json= '+vidId[0].upper()+'['+jsonLang[lang]+']['+qual+']')
                itemlist.append( Item(channel=item.channel, action='play', url=host+'/you/load-player', extra='type=movie&link='+vidId[1], title='Ver en '+vidId[0].upper()+' ['+jsonLang[lang]+']['+qual+']', thumbnail=item.thumbnail))

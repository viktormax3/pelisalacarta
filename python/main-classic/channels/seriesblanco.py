...
def mainlist(item):
    ...
    itemlist = []
    itemlist.append(Item(channel=__channel__, title="Series", action="series",
                         url=urlparse.urljoin(host, "lista_series/")))
...
def search(item, texto):
    ...
    '''
    <div style='float:left;width: 620px;'><div style='float:left;width: 33%;text-align:center;'>
        <a href='/serie/20/against-the-wall.html' '><img class='ict' src='http://4.bp.blogspot.com/-LBERI18Cq-g/
            UTendDO7iNI/AAAAAAAAPrk/QGqjmfdDreQ/s320/Against_the_Wall_Seriesdanko.jpg'
            alt='Capitulos de: Against The Wall' height='184' width='120'></a>
        <br>
        <div style='text-align:center;line-height:20px;height:20px;'>
            <a href='/serie/20/against-the-wall.html' style='font-size: 11px;'> Against The Wall</a>
        </div><br><br>
    '''

    patron = "<div style='text-align:center;line-height:20px;height:20px;'><a href='([^']+)' " \
    ...
    for scrapedurl, scrapedtitle in matches:
        itemlist.append(Item(channel=__channel__, title=scrapedtitle, url=urlparse.urljoin(host, scrapedurl),
                             action="episodios", show=scrapedtitle))
...
def episodios(item):
    ...
    </td><td> <img src=/banderas/vo.png border='0' height='15' width='25' />
    <img src=/banderas/vos.png border='0' height='15' width='25' /></td></tr>
    '''
    ...
    for scrapedurl, scrapedtitle, scrapedidioma in matches:
        ...
        itemlist.append(Item(channel=__channel__, title=title, url=urlparse.urljoin(host, scrapedurl),
                             action="findvideos", show=item.show))

    if len(itemlist) == 0 and "<title>404 Not Found</title>" in data:
        itemlist.append(Item(channel=__channel__, title="la url '" + host +
...
def findvideos(item):
    ...
    for scrapedurl, scrapedidioma, scrapedfecha, scrapedservidor, scrapeduploader, scrapedsubcalidad, scrapedtipo in \
            matches:
        title = scrapedtipo + " en " + scrapedservidor + " [" + idiomas[scrapedidioma] + "] [" + scrapedsubcalidad + \
                "] (" + scrapeduploader + ": " + scrapedfecha + ")"

...
def episodios(item):
    ...
    data = httptools.downloadpage(item.url).data
    data = re.sub(r"\n|\r|\t|\s{2}|&nbsp;", "", data)
    scrapedshow,scrapedthumbnail = scrapertools.find_single_match(data, '<h1 class="entry-title">([^"]+)<\/h1>.+?<img .+? src="([^"]+)"')
    ...
    for url, name in matches:
        ...
        if i<10:
            ...
            title = "1x"+str(i)+" "+name
        itemlist.append(item.clone(title=title, url=url, action="findvideos", show=scrapedshow, thumbnail=scrapedthumbnail))

...
def pagina_(item):
    ...
    for scrapedurl, scrapedplot, scrapedthumbnail, scrapedtitle in matches:
        ...
        itemlist.append( Item(channel=__channel__, action="episodios" , title=title , url=url, thumbnail=thumbnail, plot=scrapedplot, show=title))
...
def episodios(item):
    ...
    data = data.replace('\n','')
    data = data.replace('\r','')
    data1 = scrapertools.get_match(data, 'apitulos -->(.+?)<!-- Fin de Lista de Capitulos')
    patron  = '<a href="(.+?)".+?>(.+?)<'
    matches = re.compile(patron,re.DOTALL).findall(data1)
...
def findvideos(item):
    ...
    data = data.replace('http://ozhe.larata.in/repro-d/vk?v=','http://vk.com/video_ext.php?oid=')
...
def generos(item):
    ...
    f = open( 'C:/Users/Raul/AppData/Roaming/Kodi/addons/plugin.video.pelisalacarta/channels/dor.txt', 'w' )

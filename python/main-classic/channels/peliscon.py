...
def mainlist(item):
    ...
    itemlist.append( item.clone(title="[COLOR aqua][B]       Últimos capitulos[/B][/COLOR]", action="ul_cap",url="http://peliscon.com/episodios/",thumbnail="http://imgur.com/FrcWTS8.png",fanart="http://imgur.com/i41eduI.jpg",contentType="tvshow"))
    itemlist.append( itemlist[-1].clone(title="[COLOR crimson][B]Buscar[/B][/COLOR]", action="search",thumbnail="http://imgur.com/FrcWTS8.png",fanart="http://imgur.com/h1b7tfN.jpg"))
    return itemlist
...
def scraper(item):
    ...
    if item.contentType=="movie":
       ...
       for url,thumb,title,idioma,year in patron:
           ...
           new_item= item.clone(action="findvideos", title="[COLOR aqua]"+titulo+"[/COLOR]", url=url, fulltitle=title,contentTitle=title, contentType="movie",extra=year,library=True)
           new_item.infoLabels['year'] = year 
           itemlist.append(new_item)
       ...
       for url,thumb,title,year in patron:
           ...
           new_item = item.clone(action="findtemporadas", title="[COLOR aqua]"+titulo+"[/COLOR]", url=url, thumbnail=thumb,fulltitle=title,contentTitle=title,show=title,contentType="tvshow",library=True)
           new_item.infoLabels['year'] = year                
           itemlist.append(new_item)
...
def ul_cap(item):
    ...
    for thumb,title,url,cap in patron:
        ...
        new_item = item.clone(action="findvideos", title="[COLOR aqua]"+titulo+"[/COLOR]", url=url,thumbnail=thumb,fulltitle=title,contentTitle=title,show=title,contentType="tvshow",temp=temp,epi=epi,library=True)
...
def epis(item):
    ...
    for url,epi in patron:
        ...
        itemlist.append(item.clone( title ="[COLOR deepskyblue]Episodio "+ "[COLOR red]"+epi, url=url,  action="findvideos",show=item.show,fanart=item.extra,extra=item.extra,fanart_extra=item.fanart_extra,fanart_info=item.fanart_info, check_temp=item.check_temp,folder=True ) )
    if item.extra != "serie_add":
       tmdb.set_infoLabels_itemlist(itemlist, __modo_grafico__) 
       for item in itemlist:
    ...
    if item.contentType!="movie":
       ...
       for option, url in url_data:
           ...
           if item.temp:
               ...
               itemlist.append( Item(channel=item.channel,title = title , url=url, action="play", fanart=fanart,thumbnail= item.thumbnail,extra=item.extra,fulltitle=item.fulltitle, folder=False))
     ...
     for option, url in url_data:
         ...
         itemlist.append( Item(channel=item.channel, title = "[COLOR dodgerblue][B]"+title+" [/B][/COLOR]", url=url, action="play", fanart=item.fanart,thumbnail= item.thumbnail,extra=item.extra,InfoLabels=item.infoLabels, folder=True) )
     ...
     if item.library  and config.get_library_support() and len(itemlist) > 0 :
        ...
        itemlist.append(Item(channel=item.channel, title="Añadir esta película a la biblioteca",
            action="add_pelicula_to_library", url=item.url, fanart= item.extra.split("|")[0],infoLabels=infoLabels, text_color="0xFFe5ffcc",
